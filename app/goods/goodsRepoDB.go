...
var lockGRD = &sync.Mutex{}
var goodsRepoDB *GoodsRepoDB

func GetGoodsRepoDB() *GoodsRepo {
	lockGR.Lock()
	defer lockGR.Unlock()
	if goodsRepo == nil {
		goodsRepo = &GoodsRepo{}
...
type GoodsRepoIf interface {
	GetItemDetail(id string) *GoodsDetail
	LoadGoods() []GoodsItem
	GetGoodsList() []GoodsItem
}
type GoodsRepoDB struct {
	items []GoodsItem
}

func (gr *GoodsRepoDB) GetItemDetail(id string) *GoodsDetail {
...
func (gr *GoodsRepoDB) LoadGoods() []GoodsItem {
	if len(gr.items) != 0 {
...
func (gr *GoodsRepoDB) GetGoodsList() []GoodsItem {
	return gr.items
}

func (gr *GoodsRepoDB) createGoods(
	id uint, cateId uint,
